...
- (CGRect)expansionBackgroundRect:(UIView *)button {
    ...
    if (_fromLeft) {
        ...
        return CGRectMake(button.frame.origin.x +  button.bounds.size.width,
                          0,
    ...
    if (!_expandedButton) {
        ...
        [UIView animateWithDuration: duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if (_expansionLayout == MGSwipeExpansionLayoutCenter) {
                ...
                _expandedButton.frame = CGRectMake(_container.bounds.size.width - _expandedButton.bounds.size.width,
                                                   0,
                                                   _expandedButton.bounds.size.width,
                                                   _expandedButton.bounds.size.height);
...
#pragma clang diagnostic pop
    ...
    if (_cell.delegate && [_cell.delegate respondsToSelector:@selector(swipeTableCell:tappedButtonAtIndex:direction:fromExpansion:)]) {
        ...
        autoHide |= [_cell.delegate swipeTableCell:_cell
                               tappedButtonAtIndex:index
                                         direction:_fromLeft ? MGSwipeDirectionLeftToRight : MGSwipeDirectionRightToLeft
